#!/bin/bash
#
# Instalador do Postman - Flatpak only
#
# Comportamento:
# - Idempotente: não fará nada se o app já estiver instalado.
# - Procura por pacotes Flatpak contendo 'postman' e instala o primeiro resultado.
...
APP_NAME="Postman"
EXPECTED_BIN_NAMES=("postman" "Postman")
...
is_installed() {
    ...
    # Verifica se há um pacote flatpak instalado contendo 'postman' no nome ou na descrição
    if command -v flatpak >/dev/null 2>&1; then
        if flatpak list --app --columns=application 2>/dev/null | grep -i postman >/dev/null 2>&1; then
            echo "flatpak"
            return 0
        fi
    fi
...
info "Procurando pacotes Flatpak relacionados a 'postman'..."
# Tenta obter IDs de aplicações; adapta se a saída do flatpak search variar entre versões
search_output="$(flatpak search postman --columns=application 2>/dev/null || flatpak search postman 2>/dev/null || true)"
if [ -z "$search_output" ]; then
    warn "Nenhum resultado de busca para 'postman' via flatpak."
    echo
    echo "Verifique manualmente: flatpak search postman"
    exit 3
fi
