...
APP_NAME="DataGrip"
EXPECTED_BIN_NAMES=("datagrip" "DataGrip")
...
is_installed() {
    ...
    # Verifica se há um pacote flatpak instalado contendo 'datagrip' no nome ou na descrição
    if command -v flatpak >/dev/null 2>&1; then
        if flatpak list --app --columns=application 2>/dev/null | grep -i datagrip >/dev/null 2>&1; then
            echo "flatpak"
            return 0
        fi
    fi
...
info "Procurando pacotes Flatpak relacionados a 'datagrip'..."
# Tenta obter IDs de aplicações; adapta se a saída do flatpak search variar entre versões
search_output="$(flatpak search datagrip --columns=application 2>/dev/null || flatpak search datagrip 2>/dev/null || true)"
if [ -z "$search_output" ]; then
    warn "Nenhum resultado de busca para 'datagrip' via flatpak."
    echo
    echo "Verifique manualmente: flatpak search datagrip"
    exit 3
fi
